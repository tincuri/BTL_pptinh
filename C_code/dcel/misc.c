...
int *dfsval;
bool done = false;
double angle(struct dcel_edge *edge1,
             struct dcel_edge *edge2) { /* return the angle between 2 edges */
...
void connect(
    int x, int y,
    struct dcel_edge *h_edge) { /* function for connect node in a graph */
  struct graph_node *t;
...
int stack_empty(void) { return head->next == z; }
////////////////////////

struct node *sort_edge(struct node *f_node) { return MergeSort(f_node); }
